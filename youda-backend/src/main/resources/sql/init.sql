...
ALTER TABLE `user`
    ...
    ADD COLUMN `virtual_balance` INT DEFAULT 0 COMMENT '虚拟币余额';
...
-- 普通测试用户
INSERT INTO `user` (`username`, `password`, `nickname`, `avatar`, `phone`, `bio`, `points`, `virtual_balance`, `role`, `status`) VALUES
('student_zhang', '$2a$10$fK5GSp1tp9t4EJVZYzmZeeYO8p7XoLKxi4Y/ZsARHYkgbXWPiD.pO', '张小雨', 'https://upload.wikimedia.org/wikipedia/commons/2/26/Portrait_photography.jpg', '13900000001', '初二学生，喜欢数学和物理', 21, 120, 0, 1),
('student_li', '$2a$10$fK5GSp1tp9t4EJVZYzmZeeYO8p7XoLKxi4Y/ZsARHYkgbXWPiD.pO', '李明轩', 'https://upload.wikimedia.org/wikipedia/commons/1/1e/PORTRAIT_PICTURE.jpg', '13900000002', '高一学生，正在准备英语作文提升', 21, 88, 0, 1),
('student_wang', '$2a$10$fK5GSp1tp9t4EJVZYzmZeeYO8p7XoLKxi4Y/ZsARHYkgbXWPiD.pO', '王可心', 'https://upload.wikimedia.org/wikipedia/commons/6/6d/Portrait_woman.jpg', '13900000003', '初三学生，常整理物理和化学错题', 11, 64, 0, 1);
...
ALTER TABLE `course`
    ADD COLUMN `requires_points` TINYINT DEFAULT 0 COMMENT '是否需要购买',
    ADD COLUMN `points_cost` INT DEFAULT 0 COMMENT '课程价格（虚拟币）';
...
CREATE TABLE `course_purchase` (
    ...
    `points_cost` INT NOT NULL COMMENT '课程价格（虚拟币）',
    `status` TINYINT NOT NULL DEFAULT 1 COMMENT '订单状态：1已发货 2已收货',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='课程订单记录';
...
('初中物理受力分析专项提升（虚拟币课）', '围绕受力分析、斜面摩擦力和多物体连接模型设计的进阶课程。', 'https://upload.wikimedia.org/wikipedia/commons/4/4e/Physics_classroom.jpg', '林老师', 4, 9, 1, 0, 1, 16, DATE_SUB(NOW(), INTERVAL 2 DAY), DATE_SUB(NOW(), INTERVAL 1 DAY));
